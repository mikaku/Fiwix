...
int v1_minix_ialloc(struct inode *i)
{
	__blk_t offset;
	int inode, errno;
...
int v1_minix_truncate(struct inode *i, __off_t length)
{
	...
	if(!block || block < (BLOCKS_PER_IND_BLOCK(i->sb) + MINIX_NDIR_BLOCKS)) {
		...
		if(i->u.minix.u.i1_zone[MINIX_IND_BLOCK]) {
			free_zone(i, i->u.minix.u.i1_zone[MINIX_IND_BLOCK], block);
			if(!block) {
	...
	if(i->u.minix.u.i1_zone[MINIX_DIND_BLOCK]) {
		if(!(buf = bread(i->dev, i->u.minix.u.i1_zone[MINIX_DIND_BLOCK], i->sb->s_blocksize))) {
