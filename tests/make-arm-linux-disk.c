/*
 * Build a deterministic 1 KiB-block, revision-0 ext2 ARM Linux root.
...
int main(int argc, char **argv)
{
	...
	put16(group + 16, 3);
	mark_bits(image + INODE_BITMAP_BLOCK * BLOCK_SIZE,
		RESERVED_INODES);
	...
	mark_bit(image + INODE_BITMAP_BLOCK * BLOCK_SIZE,
		CONSOLE_INODE - 1);
	...
	init_blocks = ((unsigned long)init_size + BLOCK_SIZE - 1) /
		BLOCK_SIZE;
	if(init_blocks > 12) {
		fprintf(stderr, "init fixture is too large for direct blocks\n");
		fclose(init_input);
		free(image);
		return 2;
	}
	...
	for(logical = 0; logical < init_blocks; logical++) {
		put32(init_inode + 40 + logical * 4,
			INIT_DATA_FIRST + logical);
	}
	...
	if(linux_size <= 0 || fseek(linux_input, 0, SEEK_SET)) {
		fprintf(stderr, "invalid Linux image size in %s\n", argv[2]);
		fclose(linux_input);
		free(image);
		return 2;
	}
	linux_blocks = ((unsigned long)linux_size + BLOCK_SIZE - 1) /
		BLOCK_SIZE;
	single_block = 0;
	double_block = 0;
	double_children = 0;
	...
	if(data_first >= BLOCK_COUNT ||
		linux_blocks > BLOCK_COUNT - data_first) {
		fprintf(stderr, "Linux image is too large for the ext2 image\n");
		fclose(linux_input);
		free(image);
		return 2;
	}
	...
	if(single_block) {
		...
		for(logical = 12;
			logical < minimum(linux_blocks, 12 + 256);
			logical++) {
	...
	if(double_block) {
		...
		for(child = 0; child < double_children; child++) {
			...
			for(slot = 0; slot < 256; slot++) {
				...
				put32(image + (child_first + child) *
					BLOCK_SIZE + slot * 4, data_block);
			}
		}
	}
	...
	make_dirent(sbin_directory + 24, INIT_INODE,
		BLOCK_SIZE - 24, "init");
	...
	make_dirent(dev_directory + 24, CONSOLE_INODE,
		BLOCK_SIZE - 24, "console");
