...
INIT=${2:-arch/arm/fixture/tcc-init.elf}
...
run_tcc()
{
	transport=$1
	shift
	run_disk=$temporary/arm-tcc-$transport.img
	log=$temporary/arm-tcc-$transport.log
	output=$temporary/arm-tcc-$transport
	cp "$disk" "$run_disk"
	...
	if ! grep -q '^Fiwix ARM TinyCC process tree entered' "$log" ||
		! grep -q '^Fiwix ARM TinyCC linked ELF entered' "$log" ||
		! grep -q '^Fiwix ARM TinyCC boundary' "$log" ||
		! grep -q '^Fiwix ARM TinyCC process tree completed' "$log" ||
...
printf '%s\n' 'Fiwix ARM TinyCC compile/link/run boot passed'
